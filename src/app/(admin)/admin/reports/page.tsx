...
import { Eye, Calendar, Filter, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function AdminReportsPage() {
  ...
  const [datePickerTab, setDatePickerTab] = useState<"daily" | "period" | "tx" | "mismatch">("daily");
      ...
      <div className="sm:hidden">
        <div className="flex overflow-x-auto scrollbar-hide">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              className={`flex-1 min-w-max py-2 px-4 text-sm font-medium border-b-2 whitespace-nowrap ${
                activeTab === tab.id
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-xl p-4">
            ...
            <div className="space-y-4">
              <Input
                type="date"
                value={
                  datePickerTab === "daily" ? date :
                  datePickerTab === "period" ? sumStart :
                  datePickerTab === "tx" ? txFrom : start
                }
                onChange={(e) => {
                  const newDate = e.target.value;
                  if (datePickerTab === "daily") {
                    setDate(newDate);
                    refetchDaily();
                  } else if (datePickerTab === "period") {
                    setSumStart(newDate);
                    refetchSummary();
                  } else if (datePickerTab === "tx") {
                    setTxFrom(newDate);
                    refetchTx();
                  } else {
                    setStart(newDate);
